  ...
  return (
    <Pressable onPress={onPress} style={styles.container}>
      ...
      <Text>
        ...
        {(notification?.notificationCommentId) && (
        <Text style={{fontWeight: "bold"}} numberOfLines={1} adjustsFontSizeToFit={true} allowFontScaling={true} ellipsizeMode={"tail"}>: {notification.Comment?.comment}</Text>
      )}
      </Text>
      {(notification.Post?.image || notification.Post?.images) && (
        <Pressable onPress={navigateToPost} style={{marginLeft: 'auto'}}>
          <S3Image imgKey={notification.Post?.image ? notification.Post.image : notification?.Post?.images[0]} style={styles.image} />
        </Pressable>
      )}
